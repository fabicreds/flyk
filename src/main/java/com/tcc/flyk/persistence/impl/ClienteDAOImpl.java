	...
	public Usuario consultaUsuarioPorId(String id){
		...
		try {
			...
			if (cursor.hasNext()) {
				...
				System.out.println("Consulta de cliente pelo email " + id + " n�o encontrou valores.");
				super.desconecta();
				return null;
			}
...
/*comentado por kira 20161011
	...
	public Usuario consultaLoginById(String id) {//Este m�todo funciona, mas n�o usaremos
		try {
			super.conecta();
			DBCollection collection = db.getCollection("FLYK");
			BasicDBObject filtro = new BasicDBObject("_id", new ObjectId(id));
